<template>
    <div class="sidebar min-h-screen float-left z-50 top-0 left-0 bottom-0 sm:flex flex-col relative justify-between" :style="{ width: sidebarWidth}">
        <div>
            <div class="h-20 pt-5 flex justify-between mb-10">
                <Logo v-if="!collapsed" class="ml-4"/>
                <span
                    class="absolute top-5 right-0 p-3 ml-2.5 text-white text-3xl transition ease-linear delay-200 mr-4"
                    @click="toggleSidebar"
